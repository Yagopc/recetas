...
// Cancelar edición y volver a la receta actual
function cancelEdit() {
  showRecipe(); // Vuelve a mostrar la receta que se estaba viendo
}
...
function saveToFile() {
  // Crear el array con la nueva estructura
  const exportData = recipes.map(recipe => ({
    title: recipe.title,
    category: recipe.category,
    Cal100: 180, // Valor por defecto o puedes agregar este campo a tus recetas
  ...
  a.download = 'artículos.json'; // Nombre del archivo
  a.click();
}
...
function deleteCurrentRecipe() {
  if (currentRecipeIndex === null || currentRecipeIndex === undefined) {
    alert("No hay receta seleccionada para eliminar.");
    return;
  }
  
  const recipeTitle = recipes[currentRecipeIndex].title;
  
  if (confirm(`¿Estás seguro de que quieres eliminar la receta "${recipeTitle}"?`)) {
    recipes.splice(currentRecipeIndex, 1);
    saveToLocalStorage();
    updateRecipeCount();
    alert("Receta eliminada correctamente.");
    showMenu2(); // Volver al listado
  }
}
...
function saveRecipeToHTML() {
  ...
  // Crear el contenido HTML con mejor formato
  const htmlContent = `
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
  <div class="recipe-container">
    <h1>${recipe.title}</h1>
    <p class="category"><strong>Categoría:</strong> ${recipe.category}</p>
    
    ${recipe.image ? `
      <div class="section">
        ${recipe.image.includes('youtube.com') || recipe.image.includes('youtu.be') ? 
          `<iframe width="100%" height="315" src="https://www.youtube.com/embed/${getYouTubeId(recipe.image)}" frameborder="0" allowfullscreen></iframe>` : 
